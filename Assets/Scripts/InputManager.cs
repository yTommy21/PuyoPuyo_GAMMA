...
public class InputManager : MonoBehaviour
{
    ...
    private bool MainManager()
    {
        ...
        if (Input.GetKey(KeyCode.Mouse0))
        {
			...
			targetPos.y += concessionPosY;
			targetPos.z += concessionPosZ;
			Debug.DrawRay (Camera.main.ScreenToWorldPoint (Input.mousePosition), Vector2.one);
			//Debug.Log(targetPos);
        }

		RaycastHit2D hit = Physics2D.Raycast((Vector2)targetPos, Vector2.zero);
